...
for i=1:m
    J(i) = - Yvec(:,i)'*log(H(:,i)) - (1 - Yvec(:,i))'*log(1 - H(:,i));
end
...
o1 = Theta1(:,2:end);
o2 = Theta2(:,2:end);
J = J + (lambda/(2*m))*(sum(sum(o1.^2)) + sum(sum(o2.^2)));
...
% =========================================================================
dim = size(Theta1_grad);
for i = 1:dim(2)
    if(i == 1)
        Theta1_grad(:,i) = Theta1_grad(:,i);
    else
        Theta1_grad(:,i) = Theta1_grad(:,i) + (lambda/m)*(Theta1(:,i));
    end
end

dim = size(Theta2_grad);
for i = 1:dim(2)
    if(i == 1)
        Theta2_grad(:,i) = Theta2_grad(:,i);
    else
